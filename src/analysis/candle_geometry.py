"""Individual candle geometry: instead of named patterns (hammer/engulfing/
doji, see candles.py), this measures candle anatomy as continuous
proportions (body/wick as % of range) and the current candle's range
compared to its own recent history — simpler, quantitative price-action
rules, distinct from the named-pattern catalog:
...
git history), adapted to this project's plain-function/no-class
conventions (matching candles.py) rather than ported verbatim.
"""
