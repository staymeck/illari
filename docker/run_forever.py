...
buffer (so Binance has definitely published the candle), forever.

Which check script to run is set by the LIVE_CHECK_SCRIPT env var
(default: run_live_check.py, the original strategy — 100% equity per
trade). The second container (docker-compose.yml's
illari-live-lab-volatility service) sets it to
run_live_check_volatility.py instead — same image, same scheduling logic,
different script/state directory, so the two live approaches genuinely
run on identical fetched data at identical times.
